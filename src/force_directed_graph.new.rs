...
const ATTRACTION_FORCE: f32 = 10.0;
const REPLUSION_FORCE: f32 = 25.0;
const MASS: f32 = (std::f32::consts::TAU * NODE_SIZE) * 0.03;
        ...
        for i in 0..graph.size() {
            for j in 0..graph.size() {
                ...
                if graph.connected(Node(i), Node(j)) {
                    let ideal_dist = f32::abs(dist - IDEAL_DIST);

                    force += ATTRACTION_FORCE * f32::ln(ideal_dist);
                }
